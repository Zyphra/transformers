...
def scaled_dot_product_attention(query, key, value, attn_mask=None, dropout_p=0.0,
        is_causal=False, scale=None, enable_gqa=True) -> torch.Tensor:
...
class ZayaDynamicCache(DynamicCache):
    ...
    def __init__(self, config: Zaya1VLConfig, batch_size: int, dtype: torch.dtype = torch.float16, device: Optional[str] = None):
        super().__init__()
        self.config = config
...
class ZayaAttention(nn.Module):
    def __init__(self, config: Zaya1VLConfig, layer_n):
        ...
        if (self.use_lora_att) and ((isinstance(self.lora_rank, int) == False) or (self.lora_rank <= 0)):
            raise ValueError(
                f"lora_rank must be a positive integer."
            )
        if self.config.vision_lora:
            self.lora_linear_o = nn.Sequential(
                    nn.Linear((self.cca_num_q_heads) * self.head_dim, self.config.vision_lora_rank_attn, bias=False),#, dtype=torch.bfloat16),
    ...
    def forward(
        ...
        if self.config.vision_lora:
            query_states, key_states, value_states = self.qkv(hidden_states, past_key_values, cca_mask, image_mask=image_mask)
        else:
...
class ResidualScaling(nn.Module):
    ...
    def forward(self, residual: torch.Tensor, hidden_states: torch.Tensor):
        ...
        hidden_states = (hidden_states + self.hidden_states_bias.expand(1, 1, -1)) * self.hidden_states_scale.expand(1, 1, -1)
...
class MLP(nn.Module):
    ...
    def __init__(
        ...
        self.linear_fc1 = nn.Linear(in_features=self.config.hidden_size, out_features=ffn_hidden_size, bias=self.config.add_bias_linear)
        self.linear_fc2 = nn.Linear(in_features=ffn_hidden_size_out, out_features=self.config.hidden_size, bias=self.config.add_bias_linear)

        if self.config.vision_lora:
            self.lora_fc1 = nn.Sequential(
                nn.Linear(self.config.hidden_size, self.config.vision_lora_rank_mlp, bias=False),#, dtype=torch.bfloat16),
                nn.Linear(self.config.vision_lora_rank_mlp, ffn_hidden_size, bias=False),#, dtype=torch.bfloat16),
            )
            self.lora_fc2 = nn.Sequential(
                nn.Linear(ffn_hidden_size_out, self.config.vision_lora_rank_mlp, bias=False),#, dtype=torch.bfloat16),
                nn.Linear(self.config.vision_lora_rank_mlp, self.config.hidden_size, bias=False),#, dtype=torch.bfloat16),
            )
...
class SequentialMLP(nn.Module):
    ...
    def forward(self, permuted_local_hidden_states: torch.Tensor, tokens_per_expert: torch.Tensor, image_mask: torch.Tensor = None):
...
class ZayaBlock(nn.Module):
    ...
    def __init__(self, config, layer_idx: int, mlp_expansion: int, ffn_hidden_size: int, layer_n: int, training: bool):
        ...
        self.experts = SequentialMLP(self.num_moe_experts, self.config, ffn_hidden_size=ffn_hidden_size, layer_n=layer_n)
    ...
    def forward(self, 
        ...
        if self.config.zaya_use_mod:
            if sorted_image_mask is not None:
                expert_output, mlp_bias = self.experts(sorted_hidden_states[:sum(tokens_per_expert[:-1])], tokens_per_expert[:-1], sorted_image_mask[:sum(tokens_per_expert[:-1])])
            else:
                expert_output, mlp_bias = self.experts(sorted_hidden_states[:sum(tokens_per_expert[:-1])], tokens_per_expert[:-1]) 
                               
            expert_output = torch.cat([expert_output, sorted_hidden_states[sum(tokens_per_expert[:-1]):]], dim=0)
            if mlp_bias is not None:
                mlp_bias = torch.cat([mlp_bias, torch.zeros_like(sorted_hidden_states[sum(tokens_per_expert[:-1]):])], dim=0)
        else:
...
class ZayaDecoderMLPLayer(nn.Module):
    ...
    def __init__(self, config: Zaya1VLConfig, layer_idx: int, mlp_expansion: int, ffn_hidden_size: int, layer_n: int, training: bool):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.config.add_bias_linear:
            hidden_states, bias_states, prev_router_hidden_states = self.zaya_block(hidden_states, prev_router_hidden_states, attention_mask, image_mask, past_key_values, use_cache, cca_mask)
            hidden_states = hidden_states + bias_states
        else:
            hidden_states, _, prev_router_hidden_states = self.zaya_block(hidden_states, prev_router_hidden_states, attention_mask, image_mask, past_key_values, use_cache, cca_mask)
...
# Vision-language wrapper for Zaya1-VL. The Zaya text implementation above is local to this file;
# the Qwen2.5-VL vision transformer is intentionally imported instead of duplicated.
from dataclasses import dataclass
