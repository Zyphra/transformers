...
from .configuration_cmoe import CMoEConfig
...
class CMoERotaryEmbedding(nn.Module):
    def __init__(
        self,
        config: Optional[CMoEConfig] = None,
    ):
        super().__init__()
...
class CMoEAttention(nn.Module):
    """
    Multi-headed attention from 'Attention Is All You Need' paper. Modified to use sliding window attention: Longformer
    and "Generating Long Sequences with Sparse Transformers".
    """

    def __init__(self, config: CMoEConfig, layer_idx: Optional[int] = None):
        super().__init__()
        self.config = config
...
class CMoEFlashAttention2(CMoEAttention):
    """
    CMoE flash attention module. This module inherits from `CMoEAttention` as the weights of the module stays
...
class CMoESdpaAttention(CMoEAttention):
    """
    CMoE attention module using torch.nn.functional.scaled_dot_product_attention. This module inherits from
    `CMoEAttention` as the weights of the module stays untouched. The only changes are on the forward pass to adapt to
    SDPA API.
    """

    # Adapted from CMoEAttention.forward
    def forward(
        self,
        hidden_states: torch.Tensor,
        attention_mask: Optional[torch.Tensor] = None,
        ...
        if output_attentions:
            ...
            logger.warning_once(
                "CMoEModel is using CMoESdpaAttention, but `torch.nn.functional.scaled_dot_product_attention` does not support `output_attentions=True`. Falling back to the manual attention implementation, "
...
CMOE_ATTENTION_CLASSES = {
    "eager": CMoEAttention,
    "flash_attention_2": CMoEFlashAttention2,
    "sdpa": CMoESdpaAttention,
}


# Copied from transformers.models.mixtral.modeling_mixtral.MixtralBlockSparseTop2MLP with Mixtral->CMoE
class CMoEBlockSparseTop2MLP(nn.Module):
    def __init__(self, config: CMoEConfig):
        super().__init__()
...
class CMoESparseMoeBlock(nn.Module):
    """
    ...
    def __init__(self, config):
        ...
        self.experts = nn.ModuleList([CMoEBlockSparseTop2MLP(config) for _ in range(self.num_experts)])
...
class CMoEDecoderLayer(nn.Module):
    def __init__(self, config: CMoEConfig, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size

        self.self_attn = CMOE_ATTENTION_CLASSES[config._attn_implementation](config, layer_idx)

        self.block_sparse_moe = CMoESparseMoeBlock(config)
...
@add_start_docstrings(
    "The bare CMoE Model outputting raw hidden-states without any specific head on top.",
    CMOE_START_DOCSTRING,
)
class CMoEPreTrainedModel(PreTrainedModel):
    config_class = CMoEConfig
    base_model_prefix = "model"
    supports_gradient_checkpointing = True
    _no_split_modules = ["CMoEDecoderLayer"]
...
class CMoEModel(CMoEPreTrainedModel):
    """
    Transformer decoder consisting of *config.num_hidden_layers* layers. Each layer is a [`CMoEDecoderLayer`]
    Args:
        config: CMoEConfig
    """
    ...
    def __init__(self, config: CMoEConfig):
        ...
        self.layers = nn.ModuleList(
            [CMoEDecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
        self._attn_implementation = config._attn_implementation
        self.norm = nn.LayerNorm(config.hidden_size, eps=config.rms_norm_eps, elementwise_affine=True)
        self.rotary_emb = CMoERotaryEmbedding(config=config)
    ...
    @add_start_docstrings_to_model_forward(CMOE_INPUTS_DOCSTRING)
    def forward(
        self,
    ...
    # Copied from transformers.models.phi3.modeling_phi3.Phi3Model._update_causal_mask with Phi3->CMoE
            ...
            if attention_mask is not None and past_key_values is not None:
                ...
                if is_padding_right:
                    raise ValueError(
                        ...
                        " this may lead to unexpected behaviour for Flash Attention version of CMoE. Make sure to "
    ...
    @staticmethod
    # Copied from transformers.models.mistral.modeling_mistral.MistralModel._prepare_4d_causal_attention_mask_with_cache_position with Mistral->CMoE
...
class CMoEForCausalLM(CMoEPreTrainedModel, GenerationMixin):
    _tied_weights_keys = ["lm_head.weight"]

    def __init__(self, config):
        super().__init__(config)
        self.model = CMoEModel(config)
        self.vocab_size = config.vocab_size
    ...
    def forward(
        ...
        **loss_kwargs,
        ...
        Example:
        ```python
        >>> from transformers import AutoTokenizer, CMoEForCausalLM
        >>> model = CMoEForCausalLM.from_pretrained("Zyphra/CMoE-16B")
...
@add_start_docstrings(
    """
    The CMoE Model transformer with a sequence classification head on top (linear layer).
    [`CMoEForSequenceClassification`] uses the last token in order to do the classification, as other causal models
    (e.g. GPT-2) do.
...
# Copied from transformers.models.llama.modeling_llama.LlamaForSequenceClassification with Llama->CMoE, LLAMA->CMOE
class CMoEForSequenceClassification(CMoEPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
