...
"""Testing suite for the PyTorch CMoE model."""
...
from transformers import CMoeConfig, StaticCache, is_torch_available, set_seed
...
if is_torch_available():
    ...
    class CMoeMiniWithStaticCache(torch.nn.Module):
        def __init__(self, model: CMoeForCausalLM, batch_size: int, max_seq_len: int):
            super().__init__()
            self.model = model
        ...
        @staticmethod
        def generate(model: CMoeForCausalLM, prompt_tokens: torch.LongTensor, max_seq_len: int) -> List[int]:
            model = CMoeMiniWithStaticCache(model, 1, max_seq_len + prompt_tokens.shape[-1])
...
class CMoeModelTester:
    ...
    # Copied from tests.models.llama.test_modeling_llama.LlamaModelTester.create_and_check_model with Llama->CMoE
    def create_and_check_model(
        self, config, input_ids, token_type_ids, input_mask, sequence_labels, token_labels, choice_labels
    ):
        model = CMoEModel(config=config)
    ...
    # Copied from tests.models.llama.test_modeling_llama.LlamaModelTester.create_and_check_model_as_decoder with Llama->CMoE
    ...
    # Copied from tests.models.llama.test_modeling_llama.LlamaModelTester.create_and_check_for_causal_lm with Llama->CMoE
    def create_and_check_for_causal_lm(
        ...
        model = CMoEForCausalLM(config=config)
        model.to(torch_device)
        model.eval()
        result = model(input_ids, attention_mask=input_mask, labels=token_labels)
        self.parent.assertEqual(result.logits.shape, (self.batch_size, self.seq_length, self.vocab_size))

    # Copied from tests.models.llama.test_modeling_llama.LlamaModelTester.create_and_check_decoder_model_past_large_inputs with Llama->CMoE
...
@require_torch
class CMoEModelTest(ModelTesterMixin, GenerationTesterMixin, PipelineTesterMixin, unittest.TestCase):
    all_model_classes = (
        (CMoEModel, CMoEForCausalLM, CMoEForSequenceClassification) if is_torch_available() else ()
    )
    pipeline_model_mapping = (
        {
            "feature-extraction": CMoEModel,
            "text-classification": CMoEForSequenceClassification,
            "text-generation": CMoEForCausalLM,
            "zero-shot": CMoEForSequenceClassification,
        }
        if is_torch_available()
        else {}
    )
    ...
    # Copied from tests.models.llama.test_modeling_llama.LlamaModelTest.setUp with Llama->CMoE
    def setUp(self):
        self.model_tester = CMoEModelTester(self)
        self.config_tester = ConfigTester(self, config_class=CMoEConfig, hidden_size=37)
    ...
    # Copied from tests.models.llama.test_modeling_llama.LlamaModelTest.test_llama_sequence_classification_model with Llama->CMoE,llama->cmoe
    def test_cmoe_sequence_classification_model(self):
        config, input_dict = self.model_tester.prepare_config_and_inputs_for_common()
        config.num_labels = 3
        input_ids = input_dict["input_ids"]
        attention_mask = input_ids.ne(1).to(torch_device)
        sequence_labels = ids_tensor([self.model_tester.batch_size], self.model_tester.type_sequence_label_size)
        model = CMoEForSequenceClassification(config)
        model.to(torch_device)
        model.eval()
        result = model(input_ids, attention_mask=attention_mask, labels=sequence_labels)
        self.assertEqual(result.logits.shape, (self.model_tester.batch_size, self.model_tester.num_labels))

    # Copied from tests.models.llama.test_modeling_llama.LlamaModelTest.test_llama_sequence_classification_model_for_single_label with Llama->CMoE,llama->cmoe
    def test_cmoe_sequence_classification_model_for_single_label(self):
        config, input_dict = self.model_tester.prepare_config_and_inputs_for_common()
        config.num_labels = 3
    ...
    # Copied from tests.models.llama.test_modeling_llama.LlamaModelTest.test_llama_sequence_classification_model_for_multi_label with Llama->CMoE,llama->cmoe
    def test_cmoe_sequence_classification_model_for_multi_label(self):
        config, input_dict = self.model_tester.prepare_config_and_inputs_for_common()
        config.num_labels = 3
    ...
    def test_model_rope_scaling_from_config(self, scaling_type):
        ...
        original_model = CMoEModel(config)
...
@slow
@require_torch
class CMoEIntegrationTest(unittest.TestCase):
    def test_model_cmoe_instruct_logits(self):
        input_ids = {
        ...
        model = CMoEForCausalLM.from_pretrained("microsoft/Phi-3.5-MoE-instruct").to(torch_device)
        model.eval()
    ...
    def test_cmoe_instruct_generation(self):
        model = CMoEForCausalLM.from_pretrained("microsoft/Phi-3.5-MoE-instruct")
    ...
    def test_cmoe_instruct_with_static_cache(self):
        model = CMoEForCausalLM.from_pretrained("microsoft/Phi-3.5-MoE-instruct")
        tokenizer = AutoTokenizer.from_pretrained("microsoft/Phi-3.5-MoE-instruct")
        ...
        response_tokens = CMoEMiniWithStaticCache.generate(model, inputs, 64)
